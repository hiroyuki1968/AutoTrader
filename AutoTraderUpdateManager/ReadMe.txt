========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : AutoTraderUpdateManager
========================================================================


AppWizard が生成した AutoTraderUpdateManager には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには AutoTraderUpdateManager を構成している各ファイルの概要説明が含まれています。

AutoTraderUpdateManager.dsp
    このファイル (プロジェクト ファイル) はプロジェクト レベルの情報を含み、
    シングル プロジェクトまたはサブ プロジェクトのビルドに使用されます。他の
    ユーザーとプロジェクト ファイル (.dsp) を共有できますが、メイク ファイル
    はローカルにエクスポートしてください。

AutoTraderUpdateManager.h
    このファイルはアプリケーションの中心となるインクルードファイルです。この
    ファイルは他のプロジェクトの固有のインクルードファイル（Resource.h  も含
    みます） をインクルードし、また CAutoTraderUpdateManagerApp アプリケーションクラスを宣
    言します。

AutoTraderUpdateManager.cpp
    このファイルは CAutoTraderUpdateManagerApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。

AutoTraderUpdateManager.rc
...
res\AutoTraderUpdateManager.ico
    このファイルは、アプリケーションのアイコンとして使用されるアイコンファイ
    ルです。このアイコンはリソース ファイル AutoTraderUpdateManager.rc によってインクルード
    されます。

res\AutoTraderUpdateManager.rc2
...
AutoTraderUpdateManagerDlg.h, AutoTraderUpdateManagerDlg.cpp - ダイアログ
    これらのファイルは CAutoTraderUpdateManagerDlg クラスを含みます。このクラスは 
    アプリケーションのメイン ダイアログの動作を定義します。ダイアログ
    テンプレートは Microsoft Visual C++ で編集可能な AutoTraderUpdateManager.rc に含
    まれます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) AutoTraderUpdateManager.pch 
