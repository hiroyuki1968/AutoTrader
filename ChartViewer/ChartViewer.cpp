// ChartViewer.cpp : アプリケーション用クラスの定義を行います。
//
...
BEGIN_MESSAGE_MAP(CChartViewerApp, CWinApp)
	//{{AFX_MSG_MAP(CChartViewerApp)
...
/////////////////////////////////////////////////////////////////////////////
// CChartViewerApp クラスの構築

CChartViewerApp::CChartViewerApp()
{
...
/////////////////////////////////////////////////////////////////////////////
// 唯一の CChartViewerApp オブジェクト

CChartViewerApp theApp;

/////////////////////////////////////////////////////////////////////////////
// CChartViewerApp クラスの初期化

BOOL CChartViewerApp::InitInstance()
{
	...
	CStringBuffer strMutex = StrFormat(MUTEX_CHARTVIEWER_INSTANCE _T("_%06d"), THIS_VER) + strUlExeMode;
	...
	if (hPrevMutex) {
		...
		// 既に実行されているか
		CWnd* pWnd;
		if (pWnd = CWnd::FindWindow(NULL, strChartViewerWndName)) {
...
int CChartViewerApp::ExitInstance() 
{
...
void CChartViewerApp::WinHelp(DWORD dwData, UINT nCmd) 
{
	// TODO: この位置に固有の処理を追加するか、または基本クラスを呼び出してください
	//ここでOS等のチェックを行うとよい
...
{
	CChartViewerDlg* pChartViewerDlg = (CChartViewerDlg *)theApp.m_pMainWnd;
	...
	if ( nCode >= 0 && PM_REMOVE == wParam )
	{
		...
		)
		{
			...
			if (lpMsg->message == WM_KEYDOWN) {
				switch (lpMsg->wParam) {
				case VK_F2:
					if ((pWnd = CWnd::FindWindow(NULL, pChartViewerDlg->m_strTradeHelperWndName)))
				...
				case VK_F12:
					...
					break;
				case VK_LEFT:
					if (-- pChartViewerDlg->m_nPage < 0)
						pChartViewerDlg->m_nPage = 1 + pChartViewerDlg->m_SaSelCode.GetUpperBound() / 10;
					pChartViewerDlg->KillWMUserTimer(2);
					pChartViewerDlg->SetWMUserTimer(0, 2, NULL);
					if ((pWnd = CWnd::FindWindow(NULL, pChartViewerDlg->m_strKehaiViewerWndName)))
						pWnd->PostMessage(WM_USER + 5, pChartViewerDlg->m_nPage);
					bNull = TRUE;
					break;
				case VK_UP:
					pChartViewerDlg->m_nPage = 0;
					pChartViewerDlg->KillWMUserTimer(2);
					pChartViewerDlg->SetWMUserTimer(0, 2, NULL);
					if ((pWnd = CWnd::FindWindow(NULL, pChartViewerDlg->m_strKehaiViewerWndName)))
						pWnd->PostMessage(WM_USER + 5, pChartViewerDlg->m_nPage);
					...
					break;
				case VK_RIGHT:
					if (++ pChartViewerDlg->m_nPage > 1 + pChartViewerDlg->m_SaSelCode.GetUpperBound() / 10)
						pChartViewerDlg->m_nPage = 0;
					pChartViewerDlg->KillWMUserTimer(2);
					pChartViewerDlg->SetWMUserTimer(0, 2, NULL);
					if ((pWnd = CWnd::FindWindow(NULL, pChartViewerDlg->m_strKehaiViewerWndName)))
	...
	return CallNextHookEx(pChartViewerDlg->m_hHook, nCode, wParam, lParam);
}
